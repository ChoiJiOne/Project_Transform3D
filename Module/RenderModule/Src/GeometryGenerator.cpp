...
template <>
void GeometryGenerator::CreateCube(const Vec3f& size, std::vector<VertexPosition3D>& outVertices, std::vector<uint32_t>& outIndices)
{
	...
	for (uint32_t face = 0; face < faceCount; ++face)
	{
		...
		outVertices.push_back(VertexPosition3D(((normal - side1 - side2) * tsize)));
		outVertices.push_back(VertexPosition3D(((normal - side1 + side2) * tsize)));
		outVertices.push_back(VertexPosition3D(((normal + side1 + side2) * tsize)));
		outVertices.push_back(VertexPosition3D(((normal + side1 - side2) * tsize)));
	}
...
template <>
void GeometryGenerator::CreateCube(const Vec3f& size, std::vector<VertexPositionUv3D>& outVertices, std::vector<uint32_t>& outIndices)
{
	outVertices.resize(0);
	outIndices.resize(0);
	...
	for (uint32_t face = 0; face < faceCount; ++face)
	{
		...
		outVertices.push_back(VertexPositionUv3D(((normal - side1 - side2) * tsize), uvs[0]));
		outVertices.push_back(VertexPositionUv3D(((normal - side1 + side2) * tsize), uvs[1]));
		outVertices.push_back(VertexPositionUv3D(((normal + side1 + side2) * tsize), uvs[2]));
		outVertices.push_back(VertexPositionUv3D(((normal + side1 - side2) * tsize), uvs[3]));
	}
}


template <>
void GeometryGenerator::CreateCube(const Vec3f& size, std::vector<VertexPositionNormal3D>& outVertices, std::vector<uint32_t>& outIndices)
{
	outVertices.resize(0);
	outIndices.resize(0);
	...
	for (uint32_t face = 0; face < faceCount; ++face)
	{
		...
		outVertices.push_back(VertexPositionNormal3D(((normal - side1 - side2) * tsize), normal));
		outVertices.push_back(VertexPositionNormal3D(((normal - side1 + side2) * tsize), normal));
		outVertices.push_back(VertexPositionNormal3D(((normal + side1 + side2) * tsize), normal));
		outVertices.push_back(VertexPositionNormal3D(((normal + side1 - side2) * tsize), normal));
	}
